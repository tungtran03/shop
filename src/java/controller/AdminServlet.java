...
public class AdminServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
      SanPhamDAO bdao = new  SanPhamDAO();
        AccountDAO acdao = new  AccountDAO();
        WebDAO wdao = new WebDAO();
        List<Account> listacc = acdao.getALL();
        List<SanPham> listsp = bdao.getALL();
        List<HistoryWeb> listhistory = wdao.getHistory();
        int slPending =0;
        int sl=0;
        for (SanPham bk : listsp) {
            sl+=bk.getSlk();
        }
        Date date = new Date();
        request.setAttribute("sluser", listacc.size());
        request.setAttribute("slsanpham", listsp.size());
       
        request.setAttribute("slsptonkho", sl);
